...
const {
  ...
  mutation SendBid($bid: Float!) {
    createBid(
      input: { amount: $bid, type: "tipo", status: "status" public_session: { connect: "1" }, client: { connect: "1" }, process_item: { connect: "1" } }
    ) {
      id
      amount
      status
...
onMounted(() => {
  ...
  window.Echo.channel('bid').listen('BidCreated', ({ bid: e }) => {
    if (e.client.users[0]?.id == authStore.user.id) return
    ...
    >
      ...
      <div v-else-if="bids" class="space-y-2">
        <div
          ...
          :class="[
            'flex items-center',
            bid.client.users[0]?.id === authStore.user.id ? 'justify-end' : 'justify-start',
          ]"
        >
          <div
            :class="[
              'rounded-lg p-2 shadow max-w-xs md:max-w-sm',
              bid.client.users[0]?.id === authStore.user.id ? 'bg-blue-500 text-white' : 'bg-white text-black',
            ]"
          >
            <strong>Fornecedor {{ bid.client.users[0]?.id }}:</strong> {{ new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(bid.amount) }}
          </div>
        </div>
